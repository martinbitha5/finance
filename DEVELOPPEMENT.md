...
- **Épargne dès la paie** (`settings.savings_mode` = `none` | `amount` | `percent`, `savings_value`, `savings_auto`, réglée dans « Mon salaire ») : montant fixe ou pourcentage du salaire réellement reçu (sinon du salaire configuré). L'épargne protégée du cycle vaut le **maximum** entre ce plan et la somme des contributions mensuelles des objectifs (pas la somme, pour ne pas compter deux fois). Avec `savings_auto`, l'enregistrement d'un salaire crée aussitôt une transaction d'épargne liée (`transactions.auto_from_transaction_id`, supprimée avec le salaire), rattachée à l'objectif s'il n'y en a qu'un seul actif.
