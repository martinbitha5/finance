...
// (CT_LineChart, CT_BarChart, CT_PieChart, CT_CatAx…) : un élément mal placé
// suffit pour qu'Excel ouvre le fichier en mode « Réparé ».
// ─────────────────────────────────────────────────────────────
...
export interface ChartSpec {
  ...
  /** Couleurs des parts (camembert), une par catégorie. */
  pointColors?: string[];
...
function lineSer(i: number, s: ChartSeriesSpec, cats: ChartSpec["categories"]): string {
  return (
    ...
    `<c:spPr><a:ln w="22225" cap="rnd"><a:solidFill><a:srgbClr val="${s.color}"/></a:solidFill><a:round/></a:ln></c:spPr>` +
...
function pieSer(s: ChartSeriesSpec, cats: ChartSpec["categories"], colors: string[]): string {
  const dPts = colors
    .map((c, i) => `<c:dPt><c:idx val="${i}"/><c:bubble3D val="0"/><c:spPr><a:solidFill><a:srgbClr val="${c}"/></a:solidFill><a:ln w="12700"><a:solidFill><a:srgbClr val="FFFFFF"/></a:solidFill></a:ln></c:spPr></c:dPt>`)
    .join("");
  return (
    ...
    `<c:txPr><a:bodyPr/><a:lstStyle/><a:p><a:pPr><a:defRPr sz="900" b="1"><a:solidFill><a:srgbClr val="${INK}"/></a:solidFill><a:latin typeface="${FONT}"/></a:defRPr></a:pPr><a:endParaRPr lang="fr-FR"/></a:p></c:txPr>` +
    `<c:dLblPos val="outEnd"/><c:showLegendKey val="0"/><c:showVal val="0"/><c:showCatName val="0"/><c:showSerName val="0"/><c:showPercent val="1"/><c:showBubbleSize val="0"/><c:showLeaderLines val="1"/></c:dLbls>` +
    `<c:cat>${strRef(cats.ref, cats.labels)}</c:cat>` +
    `<c:val>${numRef(s.ref, s.values)}</c:val>` +
    `</c:ser>`
  );
...
function chartXml(spec: ChartSpec): string {
  ...
  let axes = "";
  if (spec.type === "pie") {
    plot = `<c:pieChart><c:varyColors val="1"/>${pieSer(spec.series[0]!, spec.categories, spec.pointColors ?? [])}<c:firstSliceAng val="0"/></c:pieChart>`;
  } else {
    plot =
      spec.type === "line"
    ...
    axes =
      ...
      `<c:valAx><c:axId val="${valAxId}"/><c:scaling><c:orientation val="minMax"/></c:scaling><c:delete val="0"/><c:axPos val="l"/><c:majorGridlines><c:spPr><a:ln w="6350"><a:solidFill><a:srgbClr val="E5E7EB"/></a:solidFill></a:ln></c:spPr></c:majorGridlines>${axFont}<c:crossAx val="${catAxId}"/></c:valAx>`;
  }
  ...
  return (
    ...
    axes +
    `</c:plotArea>` +
    `<c:legend><c:legendPos val="${spec.type === "pie" ? "r" : "b"}"/><c:overlay val="0"/>${axFont}</c:legend>` +
    `<c:plotVisOnly val="1"/><c:dispBlanksAs val="gap"/>` +
    `</c:chart>` +
    `<c:spPr><a:solidFill><a:srgbClr val="FFFFFF"/></a:solidFill><a:ln w="9525"><a:solidFill><a:srgbClr val="D9DBD6"/></a:solidFill></a:ln></c:spPr>` +
    `</c:chartSpace>`
  );
}
 ...
 */
export async function injectNativeCharts(xlsx: ArrayBuffer, sheetName: string, charts: ChartSpec[]): Promise<Uint8Array> {
  ...
  const chartRels = charts
    .map((_, i) => `<Relationship Id="rId${i + 1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart" Target="../charts/chart${chartIdx0 + i}.xml"/>`)
    .join("");
  zip.file(
  ...
  if (sheetRelsFile) {
    ...
    zip.file(sheetRelsPath, `${XML_HEAD}<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${drawingRel(drawingRelId)}</Relationships>`);
  }
  ...
  const overrides =
    ...
    charts.map((_, i) => `<Override PartName="/xl/charts/chart${chartIdx0 + i}.xml" ContentType="application/vnd.openxmlformats-officedocument.drawingml.chart+xml"/>`).join("");
